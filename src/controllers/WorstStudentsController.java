...
public class WorstStudentsController {
    ...
    public void initialize() throws URISyntaxException, IOException, InterruptedException {
        ...
        result.forEach(student -> {
            ...
            HBox worstStudentContainer = JavafxFunctions.createStudentContainer(fullName, studentClass,
                    Integer.toString(studentGrade),
                    student_id);

            // add them to the ArrayList `StudentHBox`
            this.students.add(new StudentHBox(worstStudentContainer, studentGrade));
        });
        ...
        Collections.sort(students, new Comparator<StudentHBox>() {
            @Override
            public int compare(StudentHBox s1, StudentHBox s2) {
        ...
        for (StudentHBox s : students) {
            worstStudentsContainer.getChildren().add(s.hbox); // Then displaying them in the UI
        }
    }
