...
public class WorstTeachersController {
    ...
    public void initialize() throws URISyntaxException, IOException, InterruptedException {
        ...
        result.forEach(teacher -> {
            ...
            ;
            HBox worstStudentContainer = JavafxFunctions.createTeacherContainer_TopAndWorstPage(fullName,
                    teacherDaysOfAbsence,
                    teacher_id);

            // add them to the ArrayList `StudentHBox`
            this.teachers.add(new TeacherentHBox(worstStudentContainer, teacherDaysOfAbsence));
        });
        ...
        Collections.sort(teachers, new Comparator<TeacherentHBox>() {
            @Override
            public int compare(TeacherentHBox s1, TeacherentHBox s2) {
        ...
        for (TeacherentHBox s : teachers) {
            worstTeachersContainer.getChildren().add(s.hbox); // Then displaying them in the UI
        }
    }
